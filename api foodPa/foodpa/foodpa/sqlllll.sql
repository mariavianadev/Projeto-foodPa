...
CREATE TABLE IF NOT EXISTS `foodPa`.`avaliacaoProduto` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `foodPa`.`avaliacaoRestaurante` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `nota` CHAR(2) NULL DEFAULT NULL,
  `idRestaurante` INT(11) NULL DEFAULT NULL,
  `idUsuario` INT(11) NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_avaliacao_restaurante1a_idx` (`idRestaurante` ASC),
  INDEX `fk_avaliacao_usuario1a_idx` (`idUsuario` ASC),
  CONSTRAINT `fk_avaliacao_restaurante1a`
    FOREIGN KEY (`idRestaurante`)
    REFERENCES `foodPa`.`restaurante` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_avaliacao_usuario1a`
    FOREIGN KEY (`idUsuario`)
    REFERENCES `foodPa`.`usuario` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
INSERT INTO `foodPa`.`restaurante` (`nome`, `cnpj`, `endereco`, `email`, `senha`, `telefone`, `categoria`)
VALUES ('Restaurante A', '123891234', 'Rua 123, Bairro ABC', 'restauranteA@email.com', 'senhaA', '9874321', 'Pizzaria' );

-- Inserir um restaurante sem especificar a foto e o telefone
INSERT INTO `foodPa`.`restaurante` (`nome`, `cnpj`, `endereco`, `email`, `senha`, `telefone`, `categoria`)
VALUES ('Restaurante B', '987101234', 'Avenida XYZ, Bairro DEF', 'restauranteB@email.com', 'senhaB', 12233313, 'Hamburgueria' );

-- Inserir um restaurante apenas com os campos obrigatórios
INSERT INTO `foodPa`.`restaurante` (`nome`, `cnpj`, `endereco`, `email`, `senha`, `telefone`, `categoria`)
VALUES ('Restaurante C', '567801234', 'Praça QWE, Bairro GHI', 'restauranteC@email.com', 'senhaC',5435435, 'Gourmet' );
...
SELECT * FROM foodPa.avaliacaoProduto;
